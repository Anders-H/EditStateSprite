...
public class SpriteRootParser
{
    ...
    public SpriteRoot Parse()
    {
        ...
        foreach (var line in _chunk)
        {
            ...
            else if (line.StartsWith("PREVIEW ANIMATION="))
            {
                var offset = line.Split('=')[1];
                result.PreviewAnimationBehaviour = PreviewAnimationBehaviourHelper.Deserialize(offset);
            }
            ...
            {
                var offset = line.Split('=')[1];
                result.X = int.Parse(offset, NumberStyles.Any);
            }
            else if (line.StartsWith("Y="))
            {
                var offset = line.Split('=')[1];
                result.Y = int.Parse(offset, NumberStyles.Any);
            }
